...
Launch the Real-Time Stream

Head to http://localhost:8501 in your browser.
Click Start System on the sidebar to initialize the AI inference models.
📐 Application Architecture
Face Mesh: Extracts 468 facial landmarks in 3D for perfect EAR ratio calculations.
YOLO Inference: Processes BGR frames locally to flag the "cell phone" object class seamlessly during the recording loop.
Session State Loop: st.empty().image() prevents the web socket from reloading the application thread and locking up the Deep Learning algorithms.
📄 License
